...
describe("files bicep", () => {
	it("should read a bicep file with valid metadata and var contentVersion", async () => {
		const { config, create, logger } = await setupTest("files bicep");
		const fileManager = new Bicep(config, logger);
...
var contentVersion string = '1.2.3'`,
	...
	it("should log a warning if unable to determine version", async () => {
		...
		expect(logger.warn).toHaveBeenCalledWith(
			"[File Manager] Unable to determine plain text version: deploy.bicep"
		);
	});

	it("should write a new version to a bicep file", async () => {
		const { config, create, logger, relativeTo } = await setupTest("files bicep");
		const fileManager = new Bicep(config, logger);
...
var contentVersion string = '4.5.6'`
	...
	it("should match bicep files", async () => {
		const { config, logger } = await setupTest("files bicep");
		const fileManager = new Bicep(config, logger);
